...
export function TrackLegend({ selectedTrack, onSelectTrack }: TrackLegendProps) {
  ...
  return (
    <div className="absolute top-4 left-4 z-10 bg-black/60 backdrop-blur-md rounded-xl p-3 flex flex-col gap-2">
      {tracks.map((track) => (
        <button
          key={track}
          ...
          className={`flex items-center gap-2 p-1.5 rounded-lg transition-all ${
            selectedTrack === track ? 'ring-2 ring-white ring-offset-1 ring-offset-black/60 bg-white/10' : ''
        ...
        >
          ...
          />
          <span className="text-sm font-medium text-white">{TRACK_LABELS[track]}</span>
        </button>
      ))}
    </div>
  );
